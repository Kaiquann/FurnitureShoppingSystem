...
$stock_sort    = req('stock_sort', 0);
$table_rows    = req('table_rows', 0);
$search        = req('search');

$sort = req('sort');
key_exists($sort, $fields) || $sort = 'stock';
...
$stock_sort_query  = get_stock_limit($stock_sort);
...
$product = new SimplePager(
    ...
    AND (p.stock BETWEEN $stock_sort_query)
    ORDER BY p.$sort $dir",
...
<ul>
    ...
    <li>
        ...
        <button type="button" data-message="Are you sure to apply the selected category?" form="select-form"
            data-action="/admin/product/update_category" class="select-btn select-submit-btn primary">Apply</button>
    </li>
    <li>
        <button type="button" data-message="Are you sure to delete all the selected product?" form="select-form"
            data-action="/admin/product/delete" class="select-btn select-submit-btn danger">Delete</button>
    </li>
</ul>
...
<ul>
    ...
    <li>
        <label for="stock_sort">Stock: </label>
        <?= html_select("stock_sort", STOCK_SORT_LIST, $stock_sort); ?>
    </li>
    <li>
...
<form>
    ...
    <input type="hidden" name="stock_sort" value="<?= $stock_sort ?>">
...
<table class="table detail">
    <thead>
        <tr>
            <?= table_headers($fields, $sort, $dir, "category_sort=$category_sort&search=$search&page=$page&price_range=$price_range&stock_sort=$stock_sort&table_rows=$table_rows") ?>
        </tr>
    </thead>
    <tbody>
        <?php if (empty($arr)) : ?>
        ...
        <?php foreach ($arr as $p) : ?>
            <tr>
                ...
                <td>
                    ...
                    <button data-post="/admin/product/delete?id=<?= $p->id ?>"
                        data-confirm="Are you sure to delete <?= html_print($p->name) ?>?" class="danger">Delete</button>
                    <div class="popup">
                        <?php
...
<?= $product->html("category_sort=$category_sort&search=$search&sort=$sort&dir=$dir&price_range=$price_range&stock_sort=$stock_sort&table_rows=$table_rows") ?>
