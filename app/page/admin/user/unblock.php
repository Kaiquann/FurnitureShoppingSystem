...
if (is_post()) {
    ...
    try {
        ...
        foreach ($id as $user_id) {
            ...
            $stmt = $_db->prepare("UPDATE users SET is_blocked = 0 WHERE id = ?");
            $stmt->execute([$user_id]);
            $stmt->closeCursor();
            ...
            $body    = generateBodyWithTitleMessage(
                $subject,
                'Your account has been unblocked by admin on date ' . getDateTime()
            );
            sendEmail($user->email, $subject, $body);
        }
        $_db->commit();
        temp('success', 'The selected user(s) have been unblocked');
    }
    catch (Exception $e) {
        $_db->rollBack();
...
return redirect('/admin/user/blocked');
